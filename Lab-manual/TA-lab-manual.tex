...
\anhkhoi{Do we want to give some percentage for attendance? See grading rubric.} \nikr{No, because that may cauee more headaches. Does this mean if they miss the session, they  can still get the other marks for the other items on the rubrik? Absense means zero for that session unless there is a valid reason, which I list in the lab instructions manual o my courses, no need for you to write that}
\anhkhoi{Ok let's discuss it at our meeting.}
...
To ensure that you are discussing the appropriate concepts during each session, {\color{blue}we expect you to either answer a question or to write down some discussion associated with text that has a blue font.} 
\underline{ Tate note of this if you intend to print the manual using black/white ink.}
...
You must motivate a proposed follow-up project by clearly stating what you expect to observe (a hypothesis) and support your claim based on your current experiment. The proposed future projects must be \textbf{realistically performed by your peers}. 
...
\nikr{The follow-up project is too complicated for this cohort. Let's discuss this last paragraph.}
\anhkhoi{I think we should keep it, I'll explain on Monday.}
...
\begin{tcolorbox}
Here we expect them to simply write an appropriate introduction and conclusion. This is more for them to write something down, to ensure that they know what they are about to do and what they'll do during the next session. 
This will help you can engage in a discussion and stimulate their critical thinking process.
...
The point system is distributed into four categories, each with a varying weights depending on the lab (since not all require the same amount of work in each lab). These weights are: unsatisfactory, minimally satisfactory, ?satisfactory and exceeding expectations (bonus points).
...
\nikr{So they don't have any questions to answer in their results part for this lab? Also, where do their refined/improved measurements for session b go? In their General Notes Section?}
\anhkhoi{Indeed, no questions. I've added a comment about how they should contrast their results from session b to those of session a.}
...
\begin{tcolorbox}
You must lead the discussion here. We want them to provide (at least) two arguments. \textbf{You must get them to reach the following conclusions.} The first is to simply measure the effective resistance and capacitance of the circuit, and show a sample of their time constant measurement.
\end{tcolorbox}
\begin{tcolorbox}
The second is to incrementally increase the resistance and capacitance separately. 
\nikr{I don't see this step anywhere in the instructions to the students. This should be linked to the last part of the paragraph below as alternate ways to fine $\tau$}
\anhkhoi{No this isn't mentioned explicitly: the TAs are supposed to initiate a discussion at the start of the session and get the students to ``come up with this idea on their own". Therefore it should be clear what they are supposed to do.}
...
The main observable of this lab is the frequency of rotation $f$ of the motor, \nikr{which  is proportional to the frequency of rotation achieved by the motor.} Similar to the first capacitance lab, you are also asked to find possible parameters that could affect the frequency of rotation of the motor.
...
Since this is the last lab session, you should try and give them less advice as they should know how to analyse linear fits. \nikr{We should modify this to inform TA to emphasis that the frequency they are measuring is related to torque, since that is not explicit in Eq. 5.4, which they never actually explicity calculate.}
\end{tcolorbox}
...
\subsection{Protocol}
\nikr{In this lab, you will find what parameters affect the frequency of rotation of the motor $f$. The magnetic field driving the motor will be that of a solenoid, and looks like the one you measured in the previous session. It has been pre-tabulated and given in  Fig.~\ref{Fig:lab3-sessionb-calibration-bfield}.}
...
\nikr{(hmm, not sure I follow, lets discuss)}
